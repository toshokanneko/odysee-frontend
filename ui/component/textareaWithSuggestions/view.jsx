...
export default function TextareaWithSuggestions(props: Props) {
  ...
  const isUriFromTermValid = suggestionTerm && !REGEX.INVALID_URI.test(suggestionTerm.substring(1));
  ...
  function handleChange(e: SyntheticInputEvent<*>) {
    ...
    const mentionMatches = value.match(REGEX.CHANNEL_MENTION);
    const emoteMatches = value.match(REGEX.EMOTE_TYPING);
    ...
    const suggestionFound =
      ...
      suggestionMatches.find((match, index) => {
        ...
        const currentMentionSearch = findNext.search(REGEX.CHANNEL_MENTION);
        const currentEmoteSearch = findNext.search(REGEX.EMOTE_TYPING);
        const currentSearch =
          currentMentionSearch >= 0 && currentEmoteSearch >= 0
            ? currentMentionSearch < currentEmoteSearch
              ? REGEX.CHANNEL_MENTION
              : REGEX.EMOTE_TYPING
            : (currentMentionSearch >= 0 && REGEX.CHANNEL_MENTION) || (currentEmoteSearch >= 0 && REGEX.EMOTE_TYPING);
